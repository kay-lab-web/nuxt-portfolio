        ...
        items: [
          {
            ...
            description02: "そこで、私がまず最初に取り掛かったのはかき小屋宇品店のリサーチです。<br>かき小屋宇品店のビジネスや、株式会社西村のグループ店や他のかき小屋との立ち位置・ユーザーとの接点を可視化し、より深く理解するのが目的です。<br><br>定性調査の面では、顕在層ユーザーに焦点を当てGoogle ビジネスプロフィール(旧:Google マイビジネス)での口コミの調査を行い、潜在層ユーザーに関しては営業部長と「かき小屋宇品店」の店長へ対し、来客層に関するヒアリングを行いました。定量面ではGoogle Search Consoleを利用し、webサイトへの流入キーワードの分析を行いました。<br><br>上記から仮定できるペルソナの策定とカスタマージャーニーマップの作成にも取り組み、「かき小屋 宇品店」の現状と課題を明確にいたしました。",
            ...
            description04: "リサーチから週末に来客する家族層に向けてのデザインを作成するにあたり、メンフィスデザインを選定いたしました。レトロリバイバルとして注目されたメンフィスデザインですが、今回選定した理由としては、<br><br><ul class='List List--disc'><li class='List__item'>図形やパターンを基調にしたポップな印象が家族層のなかでも子どもとその母親にマッチすると仮定</li><li class='List__item'>キャラクターデザインやイラストとも相性が良い</li></ul><br>上記の理由が採用理由に至りました。<br>「かき小屋宇品店」の特徴が打ち出せていないことから、トップページに「かき小屋宇品店」の特徴を分かりやすく打ち出すレイアウトにいたしました。",
            image04: "",
          ...
          },
          {
            ...
            description01: "2021年2月、入社後2か月でwebサイトを公開した後、UIの修正を行っていたところ、営業部長と然然の店長双方より「電話予約受注の負担を減らしたい」との要望を受けました。コロナ自粛期間を抜けた期間は、店舗営業のオペレーションに支障をきたすレベルで予約件数が増加傾向にありました。<br><br>定性面では社内で制作部におけるフィードバックを受けつつ、定量面ではGoogleタグマネージャ・Googleアナリティクスでの数値を計測を半年スパンで実施。下記画像のように然然webサイトのUIを改善していきました。",
            title02: "",
            description02: "",
